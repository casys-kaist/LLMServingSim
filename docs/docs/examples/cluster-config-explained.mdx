...
- TP and EP share the **same** GPUs: dense layers do TP-ALLREDUCE, MoE layers do the EP all-to-all
...
`long_prefill_token_threshold` and `block_size`.

There is deliberately no `dtype` or `kv_cache_dtype` field: every dtype
comes from the model config now, so serving two precisions of one model
means two **model configs**, one per checkpoint. See
**[Cluster config → Dtypes are not overridable](/docs/reference/cluster-config)**.
...
Full semantics, including why dtypes are not overridable and the
...
64 experts. Per-token activation crosses the DP group via the EP all-to-all.
